...
from transformers import ErnieMTokenizer, ErnieMForSequenceClassification
...
# Parameters
mn = 'ernie-m-large'
model_name = 'susnato/ernie-m-large_pytorch' # linked: https://huggingface.co/docs/transformers/en/model_doc/ernie_m
data_dir = '../data'
bs = 64
epochs = 4
lr = 1e-5
...
input = pd.read_parquet(os.path.join(data_dir, 'cls2-input.parquet'))
...
# Initialize model
model = ErnieMForSequenceClassification.from_pretrained(model_name, num_labels=2)
...
stats_df.to_parquet(os.path.join(data_dir, f'cls2-{mn}-epoch-stats-2.parquet'), index=False)
...
results_df.to_parquet(os.path.join(data_dir, f'cls2-{mn}-val-predictions-2.parquet'), index=False)

# Save model for predicting
torch.save(model.state_dict(), f'cls2-{mn}-model-state-dict-2.pth')
